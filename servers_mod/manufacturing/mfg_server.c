...
/*-------------Open PLC에서 각 컨베이어가 돌아가고 있는 지 없는 지 판단하는 정보----------------*/
static UA_NodeId RUN_MAIN_ID;
static UA_NodeId RUN_MID_ID;
static UA_NodeId RUN_LOW_ID;
static UA_NodeId FINAL_TOTAL_SET_ID;


/*----------------------------------- Modbus + OPCUA라 여기서 정의 --------------------------*/
static void onWrite_final_total_set(UA_Server *server,
    ...
    UA_UInt16 ft = *(UA_UInt16*)v->data;

    if(mb_write_reg_u16_retry(MFG_REG_FINAL_TOTAL, (uint16_t)ft) == 0) {
        printf("[MFG][MODBUS] MW10 updated by UA write: %u\n", (unsigned)ft);
    } else {
        printf("[MFG][MODBUS] MW10 update failed\n");
    }
    fflush(stdout);
}
...
static void modbus_poll_cb(UA_Server *server, void *data) {
    ...
    if(mb_read_bit_retry(MFG_COIL_RUN_MAIN, &b) == 0) {
        UA_Boolean v = b ? UA_TRUE : UA_FALSE;
        UA_Variant vv;
        UA_Variant_setScalar(&vv, &v, &UA_TYPES[UA_TYPES_BOOLEAN]);
        (void)UA_Server_writeValue(server, RUN_MAIN_ID, vv);
    }

    if(mb_read_bit_retry(MFG_COIL_RUN_MID, &b) == 0) {
        UA_Boolean v = b ? UA_TRUE : UA_FALSE;
        UA_Variant vv;
        UA_Variant_setScalar(&vv, &v, &UA_TYPES[UA_TYPES_BOOLEAN]);
        (void)UA_Server_writeValue(server, RUN_MID_ID, vv);
    }

    if(mb_read_bit_retry(MFG_COIL_RUN_LOW, &b) == 0) {
        UA_Boolean v = b ? UA_TRUE : UA_FALSE;
        UA_Variant vv;
        UA_Variant_setScalar(&vv, &v, &UA_TYPES[UA_TYPES_BOOLEAN]);
        (void)UA_Server_writeValue(server, RUN_LOW_ID, vv);
    }
}
/*-----------------------------------------------------------------------------------------------*/
...
static void production_cb(UA_Server *server, void *data) {
    ...
    if(defect) {
        ...
        g_total_count++;
                        /*---------------modbus ---------------------*/
        mb_pulse_coil(MFG_COIL_MAIN_MERGE, 120); /* 120~200ms 추천 */
        printf("[MFG][MODBUS] MAIN_MERGE pulse sent (coil=%d)\n", MFG_COIL_MAIN_MERGE);
    }
...
StartOrder_cb(UA_Server *server,
    ...
    /* ---- Modbus PLC START trigger (coil 24) ---- */
    if(mb_is_connected()) {
        mb_set_coil(MFG_COIL_START, 1);
        mb_set_coil(MFG_COIL_STOP, 0);
        printf("[MFG][MODBUS] START FORCE ON (coil=%d)\n", MFG_COIL_START);
    
    } else {
        printf("[MFG][MODBUS] not connected, start coil skipped\n");
    }
    printf("[MFG][MODBUS] START latch ON (coil=%d), STOP OFF (coil=%d)\n",
        MFG_COIL_START, MFG_COIL_STOP);
...
StopOrder_cb(UA_Server *server,
    ...
    /* ---- Modbus PLC START trigger (coil 24) ---- */
    if(mb_is_connected()) {
        mb_set_coil(MFG_COIL_START, 0);
        mb_set_coil(MFG_COIL_STOP, 1);
        printf("[MFG][MODBUS] STOP FORCE ON (coil=%d)\n", MFG_COIL_STOP);
    } else {
        printf("[MFG][MODBUS] not connected, stop coil skipped\n");
    }
    printf("[MFG][MODBUS] STOP latch ON (coil=%d), Start OFF (coil=%d)\n",
        MFG_COIL_STOP, MFG_COIL_START);
...
int main(void) {
    ...
    /* --- Modbus init (to OpenPLC) --- */
    if(mb_init("127.0.0.1", 502, 1) != 0) {
        fprintf(stderr, "[MFG][MODBUS] init failed (is OpenPLC Modbus-TCP running on 127.0.0.1:502?)\n");
        /* 여기서 바로 종료할지, 그냥 계속 갈지 선택 가능
           - 데모 안정성: return 1 권장
           - OPC UA만 먼저 띄우고 싶으면 계속 진행 */
        // return 1;
    }
    ...
    /*------------------------ModBus------------------------------*/
    RUN_MAIN_ID = UA_NODEID_STRING(1, "mfg/run_main");
    RUN_MID_ID  = UA_NODEID_STRING(1, "mfg/run_mid");
    RUN_LOW_ID  = UA_NODEID_STRING(1, "mfg/run_low");
    FINAL_TOTAL_SET_ID = UA_NODEID_STRING(1, "mfg/final_total_set");
    /*--------------------------------------------------------------*/
   ...
   /*---------------------토탈 생산량 OPCUA Client에서 받는 값 --------------------------*/
    {
    UA_VariableAttributes a = UA_VariableAttributes_default;
    a.displayName = UA_LOCALIZEDTEXT("en-US", "MFG_FinalTotalSet");
    UA_UInt16 v0 = 0;
    UA_Variant_setScalar(&a.value, &v0, &UA_TYPES[UA_TYPES_UINT16]);
    ...
    rc = UA_Server_addVariableNode(server,
        ...
        UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
        UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
        UA_QUALIFIEDNAME(1, "MFG_FinalTotalSet"),
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        a, NULL, NULL);
    }
    
    
    /*----------------------- Modbus에서 읽어온 Conv1 Conv2, Conv3가 돌아가는 지에 대한 노드 생성 -----------*/ 
    {
    UA_VariableAttributes a = UA_VariableAttributes_default;
    a.displayName = UA_LOCALIZEDTEXT("en-US", "MFG_RunMain");
    UA_Boolean v0 = UA_FALSE;
    UA_Variant_setScalar(&a.value, &v0, &UA_TYPES[UA_TYPES_BOOLEAN]);
    a.accessLevel = UA_ACCESSLEVELMASK_READ;
    a.userAccessLevel = UA_ACCESSLEVELMASK_READ;
    ...
    rc = UA_Server_addVariableNode(server,
        ...
        UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
        UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
        UA_QUALIFIEDNAME(1, "MFG_RunMain"),
        UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
        a, NULL, NULL);
    }
    {
        UA_VariableAttributes a = UA_VariableAttributes_default;
        a.displayName = UA_LOCALIZEDTEXT("en-US", "MFG_RunMid");
        UA_Boolean v0 = UA_FALSE;
        UA_Variant_setScalar(&a.value, &v0, &UA_TYPES[UA_TYPES_BOOLEAN]);
        a.accessLevel = UA_ACCESSLEVELMASK_READ;
        a.userAccessLevel = UA_ACCESSLEVELMASK_READ;
        ...
        rc = UA_Server_addVariableNode(server,
            ...
            UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
            UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
            UA_QUALIFIEDNAME(1, "MFG_RunMid"),
            UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
            a, NULL, NULL);
    }
    {
        UA_VariableAttributes a = UA_VariableAttributes_default;
        a.displayName = UA_LOCALIZEDTEXT("en-US", "MFG_RunLow");
        UA_Boolean v0 = UA_FALSE;
        UA_Variant_setScalar(&a.value, &v0, &UA_TYPES[UA_TYPES_BOOLEAN]);
        a.accessLevel = UA_ACCESSLEVELMASK_READ;
        a.userAccessLevel = UA_ACCESSLEVELMASK_READ;
    ...
    printf("[MFG] addRepeatedCallback rc=0x%08x\n", (unsigned)rc);
        ...
        /*----------------------------ModBus----------------------------------------*/
    UA_UInt64 mbPollId = 0;
    UA_StatusCode rcMb = UA_Server_addRepeatedCallback(server, modbus_poll_cb, NULL, 200 /*ms*/, &mbPollId);
    printf("[MFG] modbus poll cb rc=0x%08x\n", (unsigned)rcMb);
    
    {
    UA_ValueCallback cb;
    cb.onRead = NULL;
    cb.onWrite = onWrite_final_total_set;
    UA_Server_setVariableNode_valueCallback(server, FINAL_TOTAL_SET_ID, cb);
    }
